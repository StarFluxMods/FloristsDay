...
namespace ParentsEvent.Patches
{
    ...
    public class LayoutDecoratorPatch_FloristsLayout
    {
        static bool Prefix(LayoutDecorator __instance)
        {
            ...
            if (Profile.ID != GDOReferences.FloristsLayout.ID) return true;
            ...
            Decorator floristsDecorator = new FloristsDecorator().Setup(Blueprint, Profile, null, __instance.Decorations);
            ...
            foreach (Room room in Blueprint.Rooms())
            {
                ...
                {
                    floristsDecorator.Decorate(room);
                }
